...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            for state in states:
                ...
                if mdp.isTerminal(state):
                    ...
                    for action in allActions:
                        ...
                        if bestV <= v:
                            bestV = v
                    valueArr[state] = bestV
            self.values = valueArr
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, probability in transitions:
            ...
            d = self.discount
            newV = self.values[nextState]

            q += probability*(reward + (d*newV))
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state):
            ...
            bestA = None
            for action in allActions:
                newV = self.computeQValueFromValues(state, action)
                if bestV <= newV:
                    bestV = newV
                    bestA = action
        return bestA
