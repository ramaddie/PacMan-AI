...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            bestGhostPos = dist.argMax()
            mazeDist = self.distancer.getDistance(bestGhostPos, pacmanPosition)
            tup = (bestGhostPos, mazeDist)
            mostLikelyGhostPositions.append(tup)
        closestPos = None
        smallestDist = None
        for pos, dist in mostLikelyGhostPositions:
            if not smallestDist:
                smallestDist = dist
                closestPos = pos
