    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                oldBelief = allBeliefs[p]
                if probNoisyDistance > 0:
                    allPossible[p] = probNoisyDistance*oldBelief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            p = i % len(allPositions)
            
            self.particles.append(allPositions[p])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            for i in range(self.numParticles):
                self.particles[i] = jailPos
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                oldBelief = beliefDist[p]
                probNoisyDist = emissionModel[trueDistance]
                if probNoisyDist > 0:
                    allPossible[p] = probNoisyDist*oldBelief
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            ...
            newParticleBelief.append(newSample)
        self.particles = newParticleBelief
