...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        for i in range(len(inActs)):
            biasIA.append(inActs[i])
        wSum = self.getWeightedSum(biasIA)
        ans = self.sigmoid(wSum)
        return ans
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
            mod = alpha * delta * biasIA[i]
            newWeights.append(weight + mod)
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                val = perceptron.sigmoidActivation(currInput)
                output.append(val)
            ans.append(output)
            currInput = output
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                ...
                act = example[1]
                error = act[outputNum] - lastLayerOutput[outputNum]
                delta = error * gPrime
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    error = 0
                    dvals = deltas[0]
                    for nextNNum in range(len(nextLayer)):
                        weight = nextLayer[nextNNum].weights[neuronNum + 1]
                        nwDelta = weight * dvals[nextNNum]
                        error += nwDelta 
                    delta = error * gPrime
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                for numNeuron in xrange(len(layer)):
                    dvals = deltas[numLayer]
                    delta = dvals[numNeuron]
                    IA = allLayerOutput[numLayer]
                    weightMod = layer[numNeuron].updateWeights(IA, alpha, delta)
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while (iteration < maxItr) & (weightMod > weightChangeThreshold):
    ...
    for example in examplesTest:
        X = example[0]
        y = example[1]
        out = nnet.feedForward(X)
        hypo = out[-1]
        
        rounded = []
        for i in range(len(hypo)):
            rounded.append(round(hypo[i]))
    ...
    total = (testCorrect + testError) * 1.0
